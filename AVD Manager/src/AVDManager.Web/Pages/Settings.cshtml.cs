...
public sealed class SettingsModel : PageModel
{
    ...
    public async Task<IActionResult> OnPostSaveImageManagementAsync(CancellationToken cancellationToken)
    {
        ...
        {
            SavedAtUtc = DateTimeOffset.UtcNow,
            ImageManagementDefaults = new SavedImageManagementDefaults(ImageManagementVirtualNetworkId, ImageManagementSubnetName, ImageManagementTemporaryVmSize, Math.Clamp(ImageManagementReplicaCount, 1, 10), ImageManagementTargetRegions)
        }, cancellationToken);
        TempData["StatusMessage"] = "Image management defaults saved.";
        return RedirectToPage("/Settings");
    }
