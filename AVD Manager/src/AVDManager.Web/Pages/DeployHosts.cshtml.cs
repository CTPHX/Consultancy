...
public sealed class DeployHostsModel : PageModel
{
    ...
    public async Task<IActionResult> OnPostCancelAsync(Guid operationId, CancellationToken cancellationToken)\n    {\n        var environment = await _environmentStore.GetAsync(cancellationToken);\n        if (environment is null) return RedirectToPage("/Onboarding");\n        var operation = (await _operationStore.ListAsync(cancellationToken)).FirstOrDefault(o => o.Id == operationId);\n        if (operation is null) { ErrorMessage = "The deployment operation no longer exists."; return RedirectToPage(); }\n        if (!ActiveStatuses.Contains(operation.Status, StringComparer.OrdinalIgnoreCase)) { ErrorMessage = "This deployment workflow is no longer active."; return RedirectToPage(); }\n\n        try\n        {\n            foreach (var host in operation.OriginalHostStates ?? [])\n                await _sessionHostOperations.SetAllowNewSessionAsync(operation.SubscriptionId, operation.HostPoolResourceGroup, operation.HostPoolName, host.SessionHostName, host.AllowNewSession, cancellationToken);\n\n            await _operationStore.UpdateAsync(operation with { UpdatedAtUtc = DateTimeOffset.UtcNow, Status = "Cancelled", LastMessage = "Workflow cancelled. Original session-host drain states were restored.", ErrorMessage = null }, cancellationToken);\n            StatusMessage = "Deployment workflow cancelled and original drain states restored.";\n        }\n        catch (Exception ex)\n        {\n            ErrorMessage = $"Could not safely cancel the workflow: {ex.Message}";\n        }\n        return RedirectToPage();\n    }\n\n    public async Task<IActionResult> OnPostCheckSessionsAsync(Guid operationId, CancellationToken cancellationToken)
    {
        var environment = await _environmentStore.GetAsync(cancellationToken);
        if (environment is null) return RedirectToPage("/Onboarding");
