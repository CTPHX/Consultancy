########################################################
#Set variables
$FSLogixProfilePath1 = "\\wvdfiles01.file.core.windows.net\profiles"
#$FSLogixProfilePath2 = "\\wvdfiles01.file.core.windows.net\profiles"
#$FSLogixProfilePath3 = "\\wvdfiles01.file.core.windows.net\profiles"
$AppAttachPath = "\\wvdfiles01.file.core.windows.net\profiles"

$AADTenantID  = "xxxxx-xxxxx-xxxxxx-xxxxxxx-xxxxxx"
########################################################
...
if (!(Test-Path "C:\WVD\Language.txt")) {
...
if (!(test-path -path C:\WVD\Language)) {new-item -path C:\WVD\Language -itemtype directory}
if (!(test-path -path C:\WVD\Language\en-gb)) {new-item -path C:\WVD\Language\en-gb -itemtype directory}

Write-Host '*** Download lange files from blob storage ***'

Invoke-WebRequest -Uri 'https://phxmbwvd.blob.core.windows.net/mbautomation/Language.zip' -OutFile 'c:\temp\language.zip' -ErrorAction Stop
Expand-Archive -Path 'C:\temp\language.zip' -DestinationPath 'C:\WVD\'  -Force
...
#Start-Sleep -Seconds 10
# Copy files to C:\WVD
#Copy-Item "X:\LocalExperiencePack\en-gb\LanguageExperiencePack.en-GB.Neutral.appx" "c:\wvd\language\en-gb"
#Copy-Item "X:\LocalExperiencePack\en-gb\License.xml" "C:\wvd\language\en-gb"
#Copy-Item "X:\x64\Langpacks\Microsoft-Windows-Client-Language-Pack_x64_en-gb.cab" "c:\wvd\language"
...
#Start-Sleep -Seconds 10
# Copy files to C:\WVD
#get-childitem -Recurse -path "Y:\" -filter '*en-gb*' | Copy-Item -Destination "C:\WVD\Language"

#Unmount drive
#DisMount-DiskImage -ImagePath $isoImg

##Set Language Pack Content Stores##
[string]$LIPContent = "C:\WVD\Language"
...
$RegionalSettings = "C:\WVD\Region.xml"

if (!(test-path -path c:\wvd\Region.xml)) {new-item -path c:\wvd -name Region.xml -ItemType File -Value '
...
Write-Output 'language pack installed' | Out-File 'c:\wvd\Language.txt' -Append
}
############# END OF LANGUAGE PACK INSTALLATION ############

Write-Host '*** WVD Customisation **************************************************************************************************'
Write-Host '*** WVD Customisation ***                                                                                            ***'
Write-Host '*** WVD Customisation *** Script: Win10ms_O365.ps1                                                                   ***'
Write-Host '*** WVD Customisation ***                                                                                            ***'
Write-Host '*** WVD Customisation **************************************************************************************************'

Write-Host '*** WVD Customisation *** Stop the custimization when Error occurs ***'
#$ErroractionPreference='Stop'
$ErroractionPreference='Continue'

Write-Host '*** WVD Customisation *** CONFIG *** Create temp folder for software packages. ***'
if (!(test-path -path c:\temp)) {new-item -path c:\temp -itemtype directory}
#New-Item -Path 'C:\temp' -ItemType Directory -Force | Out-Null

Write-Host '*** WVD Customisation *** INSTALL *** Install FSLogix ***'
...
#New-ItemProperty "HKLM:\software\FSLogix\Profiles" -Name "RedirXMLSourceFolder" -Value "c:\WVD\FSLogix" -PropertyType String -Force | Out-Null
#add the following:
...
#####################################################################################
#This section configures Microsoft best practice settings for WVD#
#####################################################################################

Write-Host '*** WVD Customisation *** START OS CONFIG *** Update the recommended OS configuration ***'
Write-Host '*** WVD Customisation *** SET OS REGKEY *** Disable Automatic Updates ***'
...
Write-Host '*** WVD Customisation *** SET OS REGKEY *** Prevent Microsoft Edge first run page***'
reg add "HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\MicrosoftEdge\Main" /v PreventFirstRunPage /t REG_DWORD /d 1 /f

Write-Host '*** WVD Customisation *** SET OS REGKEY *** Specify Start layout for Windows 10 PCs (optional) ***'
...
Write-Host '*** WVD Customisation *** SET OS REGKEY *** Set up time zone redirection ***'
...
Write-Host '*** WVD Customisation *** SET OS REGKEY *** Disable Storage Sense ***'
...
Write-Host '*** WVD Customisation *** SET OS REGKEY *** For feedback hub collection of telemetry data on Windows 10 Enterprise multi-session ***'
...
# Note: Remove if not required!
Write-Host '*** WVD Customisation *** SET OS REGKEYS *** Fix 5k resolution support ***'
...
#Write-Host '*** WVD Customisation *** SET OS REGKEY *** Temp fix for 20H1 SXS Bug ***'
...
Write-Host '*** WVD Customisation *** START OFFICE CONFIG *** Config the recommended Office configuration ***'
Write-Host '*** WVD Customisation *** CONFIG OFFICE Regkeys *** Mount default registry hive ***'
& REG LOAD HKLM\DEFAULT C:\Users\Default\NTUSER.DAT
Start-Sleep -Seconds 5
Write-Host '*** WVD Customisation *** CONFIG OFFICE *** Set InsiderslabBehavior ***'
...
Write-Host '*** WVD Customisation *** CONFIG OFFICE *** Set Outlooks Cached Exchange Mode behavior ***'
...
Write-Host '*** WVD Customisation *** CONFIG OFFICE Regkeys *** Un-mount default registry hive ***'
[GC]::Collect()
& REG UNLOAD HKLM\DEFAULT
Start-Sleep -Seconds 5

Write-Host '*** WVD Customisation *** CONFIG OFFICE Regkeys *** Set Office Update Notifiations behavior ***'
...
Write-Host '*** WVD Customisation *** INSTALL *** Install C++ Redist for RTCSvc (Teams Optimized) ***'
...
Write-Host '*** WVD Customisation *** INSTALL *** Install RTCWebsocket to optimize Teams for WVD ***'
...
Write-Host '*** WVD Customisation *** INSTALL *** Install Teams in Machine mode ***'
...
Invoke-Expression -Command 'msiexec /i C:\temp\Teams.msi /quiet /l*v C:\WVD\teamsinstall.log ALLUSER=1 ALLUSERS=1 OPTIONS="noAutoStart=true"'
Write-Host '*** WVD Customisation *** CONFIG TEAMS *** Configure Teams to start at sign in for all users. ***'
...
Invoke-WebRequest -Uri 'https://github.com/The-Virtual-Desktop-Team/Virtual-Desktop-Optimization-Tool/archive/refs/heads/main.zip' -OutFile 'c:\WVD\master.zip' -ErrorAction Stop
...
((Get-Content -Path C:\WVD\Virtual-Desktop-Optimization-Tool-main\2009\ConfigurationFiles\AppxPackages.json -Raw) -replace 'Microsoft.MSPaint', 'Microsoft.MSPaint_DonNotRemove') | Set-Content -Path C:\WVD\Virtual-Desktop-Optimization-Tool-main\2009\ConfigurationFiles\AppxPackages.json
((Get-Content -Path C:\WVD\Virtual-Desktop-Optimization-Tool-main\2009\ConfigurationFiles\AppxPackages.json -Raw) -replace 'Microsoft.MicrosoftStickyNotes', 'Microsoft.MicrosoftStickyNotes_DonNotRemove') | Set-Content -Path C:\WVD\Virtual-Desktop-Optimization-Tool-main\2009\ConfigurationFiles\AppxPackages.json
((Get-Content -Path C:\WVD\Virtual-Desktop-Optimization-Tool-main\2009\ConfigurationFiles\AppxPackages.json -Raw) -replace 'Microsoft.Windows.Photos', 'Microsoft.Windows.Photos_DonNotRemove') | Set-Content -Path C:\WVD\Virtual-Desktop-Optimization-Tool-main\2009\ConfigurationFiles\AppxPackages.json
((Get-Content -Path C:\WVD\Virtual-Desktop-Optimization-Tool-main\2009\ConfigurationFiles\AppxPackages.json -Raw) -replace 'Microsoft.WindowsAlarms', 'Microsoft.WindowsAlarms_DonNotRemove') | Set-Content -Path C:\WVD\Virtual-Desktop-Optimization-Tool-main\2009\ConfigurationFiles\AppxPackages.json
((Get-Content -Path C:\WVD\Virtual-Desktop-Optimization-Tool-main\2009\ConfigurationFiles\AppxPackages.json -Raw) -replace 'Microsoft.WindowsCalculator', 'Microsoft.WindowsCalculator_DonNotRemove') | Set-Content -Path C:\WVD\Virtual-Desktop-Optimization-Tool-main\2009\ConfigurationFiles\AppxPackages.json
((Get-Content -Path C:\WVD\Virtual-Desktop-Optimization-Tool-main\2009\ConfigurationFiles\AppxPackages.json -Raw) -replace 'Microsoft.WindowsCamera', 'Microsoft.WindowsCamera_DonNotRemove') | Set-Content -Path C:\WVD\Virtual-Desktop-Optimization-Tool-main\2009\ConfigurationFiles\AppxPackages.json
((Get-Content -Path C:\WVD\Virtual-Desktop-Optimization-Tool-main\2009\ConfigurationFiles\AppxPackages.json -Raw) -replace 'Microsoft.WindowsSoundRecorder', 'Microsoft.WindowsSoundRecorder_DonNotRemove') | Set-Content -Path C:\WVD\Virtual-Desktop-Optimization-Tool-main\2009\ConfigurationFiles\AppxPackages.json
((Get-Content -Path C:\WVD\Virtual-Desktop-Optimization-Tool-main\2009\ConfigurationFiles\AppxPackages.json -Raw) -replace 'Microsoft.ScreenSketch', 'Microsoft.ScreenSketch_DonNotRemove') | Set-Content -Path C:\WVD\Virtual-Desktop-Optimization-Tool-main\2009\ConfigurationFiles\AppxPackages.json

((Get-Content -Path C:\WVD\Virtual-Desktop-Optimization-Tool-main\2009\ConfigurationFiles\Services.json -Raw) -replace 'UsoSvc', 'UsoSvc_DonNotStop') | Set-Content -Path C:\WVD\Virtual-Desktop-Optimization-Tool-main\2009\ConfigurationFiles\Services.json
((Get-Content -Path C:\WVD\Virtual-Desktop-Optimization-Tool-main\2009\ConfigurationFiles\Services.json -Raw) -replace 'DiagTrack', 'DiagTrack_DonNotStop') | Set-Content -Path C:\WVD\Virtual-Desktop-Optimization-Tool-main\2009\ConfigurationFiles\Services.json
...
#Creating Scheduled Task for WVD Optimisation 
if(Get-ScheduledTask 'WVD Customisation' -ErrorAction Ignore) {Write-Output "Scheduled Task already created"} 
else 
{
# Create a new task action
$taskAction = New-ScheduledTaskAction `
    -Execute 'C:\WINDOWS\system32\WindowsPowerShell\v1.0\powershell.exe' `
    -Argument '-ExecutionPolicy Bypass -command "& c:\WVD\Optimise.ps1"'
$taskAction
...
$taskName = "WVD Customisation"

# Describe the scheduled task.
$description = "Customise and Optimise a WVD Session Host"
...
Set-ScheduledTask -TaskName 'WVD Customisation' -Principal $taskPrincipal -Settings $taskSettings
}
if (!(test-path -path c:\wvd\FSLogix)) {new-item -path c:\wvd\FSLogix -itemtype directory}
if (!(test-path -path c:\wvd\FSLogix\Redirections.xml)) {new-item -path c:\wvd\FSLogix -name Redirections.xml -ItemType File -Value '
...
Write-Host '*** WVD Customisation *** CONFIG *** Deleting temp folder. ***'
if (test-path -path c:\temp) {
Get-ChildItem -Path 'C:\temp' -Recurse | Remove-Item -Recurse -Force
Remove-Item -Path 'C:\temp' -Force | Out-Null
}
Write-Host '*** WVD Customisation *** CONFIG *** Deleting Language folder. ***'
if (test-path -path c:\wvd\Language) {
Get-ChildItem -Path 'C:\WVD\Language' -Recurse | Remove-Item -Recurse -Force
Remove-Item -Path 'C:\WVD\Language' -Force | Out-Null
} 
