...
Only deployment-time controls that are useful to AVD Manager remain in the param block.
...
SAFETY BEHAVIOUR
- OverwriteExisting always places all existing session hosts into drain mode before replacement.
- If user sessions still exist and ForceLogoffUsers is False, replacement stops before deleting any hosts.
- If ForceLogoffUsers is True, existing user sessions are forcibly logged off and then re-checked before deletion.
- If replacement is blocked because sessions exist, original drain-mode states are restored where possible.
...
- GalleryImageVersion defaults to Latest. A specific gallery version such as 0.0.1 may be supplied.
- VmSize defaults to Standard_D2ds_v6 and can be overridden by AVD Manager.
...
function Test-DeploymentInputs {
    ...
    if ([string]::IsNullOrWhiteSpace($HostPoolName)) {
        throw "HostPoolName cannot be empty."
    }

    if ([string]::IsNullOrWhiteSpace($VmNamePrefix)) {
        throw "VmNamePrefix cannot be empty."
    }
    ...
    if ($ForceLogoffUsers -and -not $OverwriteExisting) {
        Write-Log -Level 'WARN' -Component 'VALIDATION' -Message 'ForceLogoffUsers is True but OverwriteExisting is False. The logoff option will be ignored because no existing hosts are being replaced.'
    }
...
function Restore-OriginalSessionHostDrainStates {
    param(
        [object[]]$SessionHostStates
    )

    if (-not $SessionHostStates) {
        return
    }

    Write-Log -Level 'INFO' -Component 'AVD' -Message 'Restoring original drain-mode states because replacement is not proceeding...'
    ...
    foreach ($state in $SessionHostStates) {
        try {
            Update-AzWvdSessionHost `
                ...
                -AllowNewSession:$state.OriginalAllowNewSession | Out-Null

            Write-Log -Level 'INFO' -Component 'AVD' -Message ("Restored '{0}' to AllowNewSession={1}." -f $state.SessionHostName, $state.OriginalAllowNewSession)
        }
        catch {
            Write-Log -Level 'WARN' -Component 'AVD' -Message ("Could not restore original drain state for '{0}': {1}" -f $state.SessionHostName, $_.Exception.Message)
        }
    }
}
...
function Invoke-DrainAndDeleteExistingHosts {
    ...
    foreach ($sessionHost in $sessionHosts) {
        ...
        $sessionHostStates += [pscustomobject]@{
            ...
            VmName                  = $vmName
            OriginalAllowNewSession = [bool]$sessionHost.AllowNewSession
            UserSessions            = @()
        }
    }

    Write-Log -Level 'WARN' -Component 'AVD' -Message ("Replacement requested. Placing {0} existing session host(s) into drain mode before checking user sessions." -f $sessionHostStates.Count)
    ...
    foreach ($state in $sessionHostStates) {
        try {
            ...
            Write-Log -Level 'INFO' -Component 'AVD' -Message ("Drain mode enabled for '{0}'." -f $state.SessionHostName)
        }
        catch {
            Restore-OriginalSessionHostDrainStates -SessionHostStates $sessionHostStates
            Throw-RunbookError -Step "DRAIN_SESSION_HOST_$($state.SessionHostName)" -ErrorRecord $_
        }
    }
    ...
    foreach ($state in $sessionHostStates) {
        ...
        foreach ($userSession in $state.UserSessions) {
    ...
    if ($sessionsFound.Count -gt 0 -and -not [bool]$ForceLogoffUsers) {
        $sessionSummary = @(
            $sessionsFound |
                ForEach-Object { "{0}: {1}" -f $_.SessionHostName, $_.UserLabel }
        ) -join '; '

        Write-Log -Level 'WARN' -Component 'AVD' -Message ("Replacement stopped because {0} user session(s) are still present. No session hosts or VMs have been deleted." -f $sessionsFound.Count)
        Restore-OriginalSessionHostDrainStates -SessionHostStates $sessionHostStates
        throw "Existing user sessions prevent replacement. $sessionSummary. Rerun with ForceLogoffUsers=True only when those users may be forcibly logged off."
    }

    if ($sessionsFound.Count -gt 0 -and [bool]$ForceLogoffUsers) {
        Write-Log -Level 'WARN' -Component 'AVD' -Message ("ForceLogoffUsers is True. Logging off {0} existing user session(s) before replacement." -f $sessionsFound.Count)

        foreach ($state in $sessionHostStates) {
            foreach ($userSession in $state.UserSessions) {
                Write-Log -Level 'WARN' -Component 'AVD' -Message ("Logging off session id '{0}' from '{1}'..." -f $userSession.Id, $state.SessionHostName)
        ...
        foreach ($state in $sessionHostStates) {
            ...
            foreach ($remaining in $remainingForHost) {
                $remainingSessions += $remaining
            }
        }

        if ($remainingSessions.Count -gt 0) {
            Write-Log -Level 'ERROR' -Component 'AVD' -Message ("Replacement stopped because {0} user session(s) remained after forced logoff. No session hosts or VMs have been deleted." -f $remainingSessions.Count)
            Restore-OriginalSessionHostDrainStates -SessionHostStates $sessionHostStates
            throw "One or more user sessions remained after forced logoff. Replacement has been stopped."
        }

        Write-Log -Level 'SUCCESS' -Component 'AVD' -Message 'All existing user sessions were logged off successfully.'
    }
    else {
        Write-Log -Level 'SUCCESS' -Component 'AVD' -Message 'No existing user sessions were found. Replacement may proceed.'
    }
...
try {
    ...
    Write-Log -Level 'INFO' -Component 'PLAN' -Message ("Image version      : {0}" -f $GalleryImageVersion)
    Write-Log -Level 'INFO' -Component 'PLAN' -Message ("OverwriteExisting  : {0}" -f ([bool]$OverwriteExisting))
    Write-Log -Level 'INFO' -Component 'PLAN' -Message ("Force logoff users : {0}" -f ([bool]$ForceLogoffUsers))
    ...
    if ([bool]$OverwriteExisting) {
        Write-Log -Level 'WARN' -Component 'PLAN' -Message "OverwriteExisting is True. Existing session hosts will be drained first. Replacement will stop if user sessions remain unless ForceLogoffUsers is explicitly True."
    ...
    Write-Log -Level 'SUCCESS' -Component 'PLAN' -Message 'Generated VM names passed Windows computer-name validation.'
